...
export default function App() {
  const [data, setData] = useState<AppData>(() => {
    const saved = localStorage.getItem('timbo-em-missao-v1');
    return saved ? JSON.parse(saved) : requireInit();
  });
  ...
  const persist = (next: AppData) => {
    setData(next);
    localStorage.setItem('timbo-em-missao-v1', JSON.stringify(next));
  };
  ...
  const recent = [...data.donations].sort((a, b) => b.dataCriacao.localeCompare(a.dataCriacao)).slice(0, 6);
  ...
  function editFood(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); if (!selected) return; const fd = new FormData(e.currentTarget);
    const nome = String(fd.get('nome')).trim(), peso = Number(fd.get('peso')), meta = Number(fd.get('meta'));
    if (!nome || peso <= 0 || meta <= 0) return notify('Preencha os campos com valores válidos.');
  ...
  function addDonation(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); const fd = new FormData(e.currentTarget), alimentoId = String(fd.get('alimentoId')), quantidade = Number(fd.get('quantidade'));
  ...
  function saveCampaign(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); const fd = new FormData(e.currentTarget), name = String(fd.get('name')).trim(), meta = Number(fd.get('meta')), desc = String(fd.get('description')).trim();
...
function requireInit():AppData { return { campaign:{id:crypto.randomUUID(),name:'Timbó em Missão — Ação Social 2026',metaKg:3000,dataCriacao:new Date().toISOString(),description:'Uma ação social da igreja para levar cuidado, alimento e esperança a quem precisa.'}, foods:[], donations:[] }; }
